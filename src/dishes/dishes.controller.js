...
function pricePropertyGreaterThanZero(req, res, next){
    const {data: {price} = {}} = req.body;
    if(price === 0 || price < 0){
        next({
            status: 400,
            message: "Dish must have a price that is an integer greater than 0ish must include a description."
        })
    }
    return next();
}
